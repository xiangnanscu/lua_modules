...
function array.exclude(t, callback)
  local res = array:new()
  for i = 1, #t do
    if not callback(t[i], i, t) then
      res[#res + 1] = t[i]
...
function array.count(t, callback)
  local res = 0
  for i = 1, #t do
    if callback(t[i], i, t) then
      res = res + 1
...
function array.count_exclude(t, callback)
  local res = 0
  for i = 1, #t do
    if not callback(t[i], i, t) then
      res = res + 1
...
function array.combine(t, n)
  if #t == n then
    return array {t}
  elseif n == 1 then
    return array.map(t, function(e)
      return array {e}
    end)
  elseif #t > n then
    local head = t[1]
    local rest = array.slice(t, 2)
