...
local function __call(t, object, opts)
  return t:new(opts):go(object)
end
...
setmetatable(repr, {__call = __call})
repr.__index = repr
repr.__call = __call
function repr.new(cls, self)
  return setmetatable(self or {}, cls)
end
function repr.s(cls, o)
  return cls:new{hide_address = false}:go(type(o) == "table" and o or symbol(o))
end
function repr.doArray(self, t, indent, deep, visited)
  ...
  for i, v in ipairs(t) do
    res[#res + 1] = string.format("%s%s,", key_indent, self:go(v, indent + self.indent, deep + 1, visited))
  end
...
function repr.doDict(self, t, indent, deep, visited)
  ...
  for k, v in rawpairs(t) do
    ...
    normalize[k] = self:go(v, indent + self.indent, deep + 1, visited)
  end
  t = normalize
  local key_indent = make_spaces(indent)
...
function repr.go(self, o, indent, deep, visited)
